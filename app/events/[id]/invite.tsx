/**
 * Event Invite Screen
 * Allows event creators and attendees to invite other users to an event
 */
...
import { eventsAPI, searchAPI } from '../../../src/lib/apiClient';
...
export default function EventInviteScreen() {
  ...
  const inviteMutation = useMutation({
    mutationFn: async (userIds: number[]) => {
      return eventsAPI.inviteUsers(eventId, userIds, true);
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ['event', eventId] });
      const successCount = data.results?.filter((r: any) => r.status === 'invited').length || selectedUsers.length;
      Alert.alert(
        'Invitations Sent',
  ...
  const toggleUserSelection = useCallback((user: SearchUser) => {
    ...
    >
      ...
      {isSearching ? (
        ...
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptyText}>Invite friends to this event</Text>
          <Text style={styles.emptySubtext}>Search for users by name or username</Text>
        </View>
