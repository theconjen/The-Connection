...
async function debug() {
  ...
  console.log('first body items length', Array.isArray(res.body.items) ? res.body.items.length : 'legacy array');

  // traverse a few pages
  let page = res.body;
  for (let i = 0; i < 5; i++) {
    if (!page || !Array.isArray(page.items)) break;
    console.log(`page ${i} items ${page.items.length} nextCursor=${page.nextCursor}`);
