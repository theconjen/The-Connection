...
import { users, apologeticsTopics, apologeticsQuestions, apologeticsAnswers } from "../shared/schema.js";
import { eq } from "drizzle-orm";
async function seedApologetics() {
  console.log("Starting apologetics data seeding...");
  try {
    const existingTopics = await db.select({ count: sql`count(*)` }).from(apologeticsTopics);
    ...
    if (demoUser) {
      await db.update(users).set({ isVerifiedApologeticsAnswerer: true }).where(eq(users.id, demoUser.id));
    ...
    for (const answer of insertedAnswers) {
      ...
      if (question) {
        await db.update(apologeticsQuestions).set({ answerCount: 1, status: "answered" }).where(eq(apologeticsQuestions.id, question.id));
      }
    }
