...
import { users, posts, comments, communities, microblogs } from "../shared/schema.js";
...
async function seedFeed() {
  ...
  try {
    ...
    for (const community of allCommunities) {
      ...
      if (postCount > 0) {
        await db.update(communities).set({ memberCount: community.memberCount || 1 }).where(eq(communities.id, community.id));
      }
    }
